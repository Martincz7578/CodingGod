...
import { Building, citizen } from "./classes.js";
...
export function process(
  ...
  if (frame % 15 === 0) {
    //every second
    demands.raw.coal = Math.floor(
      priceChange(
        resources.raw.coal,
        mines ? mines : 1,
        shopsOpenned && shops > 0 ? shopsOpenned : false
      )
    );
    demands.raw.iron = Math.floor(
      priceChange(
        resources.raw.iron,
        mines ? mines : 1,
        shopsOpenned && shops > 0 ? shopsOpenned : false
    ...
    demands.processed.refinedCoal = Math.floor(
      priceChange(
        resources.processed.refinedCoal,
        foundries ? foundries : 1,
        shopsOpenned && shops > 0 ? shopsOpenned : false
      )
    );
    demands.processed.steel = Math.floor(
      priceChange(
        resources.processed.steel,
        foundries ? foundries : 1,
        shopsOpenned && shops > 0 ? shopsOpenned : false
      )
    );
    demands.processed.stoneBricks = Math.floor(
      priceChange(
        resources.processed.stoneBricks,
        masons ? masons : 1,
        shopsOpenned && shops > 0 ? shopsOpenned : false
      )
    );
    ...
    playerStats.raw.coal +=
      mines *
      (buildings.mines?.productionSpeed ?? 0) *
      productionAmplifiers.mines;
    playerStats.raw.iron +=
      mines *
      (buildings.mines?.productionSpeed ?? 0) *
      productionAmplifiers.mines;
    playerStats.raw.stone +=
      mines *
      (buildings.mines?.productionSpeed ?? 0) *
      productionAmplifiers.mines;
    //processing
    ...
    if (
      playerStats.raw.coal >=
      foundries *
        (buildings.foundry?.productionSpeed ?? 0) *
        productionAmplifiers.foundries
    ) {
      playerStats.processed.refinedCoal +=
        foundries *
        (buildings.foundry?.productionSpeed ?? 0) *
        productionAmplifiers.foundries;
      playerStats.raw.coal -=
    ...
    if (
      playerStats.raw.stone >=
      masons *
        (buildings.mason?.productionSpeed ?? 0) *
        productionAmplifiers.masons
    ) {
      playerStats.processed.stoneBricks +=
        masons *
        (buildings.mason?.productionSpeed ?? 0) *
        productionAmplifiers.masons;
      playerStats.raw.stone -=
    ...
    if (shopsOpenned && shops > 0) {
      if (playerStats.processed.steel > demands.processed.steel * shops) {
        const steelIncome =
          demands.processed.steel * (resources.processed.steel.price * shops);
      ...
      if (
        playerStats.processed.stoneBricks >
        demands.processed.stoneBricks * shops
      ) {
        const stoneBricksIncome =
          demands.processed.stoneBricks *
          (resources.processed.stoneBricks.price * shops);
    ...
    //hunger process
    playerStats.food += farms * (buildings.farm?.productionSpeed ?? 0);
    if (playerStats.food >= citizens.length) {
      playerStats.food -= citizens.length;
    } else if (!populationData.hunger) {
      mpop(
        "You have run out of food to feed your population! Build farms to produce more food.",
        "Uh oh"
      );
    ...
    placedBuildings.forEach((building) => {
      if (building.data.type === buildingTypes.HOUSE) {
        if (
          building.householdMembers!.length < building.maxMembers! &&
          building.householdMembers![0] != undefined
        ) {
          Math.random() < 0.5
            ? condicionedBirth(
                1,
                building.householdMembers![0]!.happiness,
                building.householdMembers![0]!.hunger,
                building
              )
            : null;
        }
...
export function condicionedBirth(
  births: number,
  happiness: number,
  hunger: number,
  household: Building
...
function priceChange(
  ...
  const logistic =
    maxDemand / (1 + Math.exp(-resource.k * (frame - resource.t0)));
  const decay = Math.exp(
    -resource.d *
      Math.max(0, optimizeUnits(frame) - optimizeUnits(resource.tsat))
  );
...
function updateStatsDisplay() {
  playerStats.raw.total =
    playerStats.raw.coal + playerStats.raw.iron + playerStats.raw.stone;
  playerStats.processed.total =
    playerStats.processed.refinedCoal +
    playerStats.processed.steel +
    playerStats.processed.stoneBricks;
  rawSpan.innerText = `Raw: ${playerStats.raw.total.toFixed(0)}`;
  processedSpan.innerText = `Processed: ${playerStats.processed.total.toFixed(
    0
  )}`;
...
function popupData() {
  rawInfo.innerHTML = `Raw:<br>
    Coal: ${playerStats.raw.coal.toFixed(
      0
    )}, Price: ${resources.raw.coal.price.toFixed(2)}<br>
    Iron: ${playerStats.raw.iron.toFixed(
      0
    )}, Price: ${resources.raw.iron.price.toFixed(2)}<br>
    Stone: ${playerStats.raw.stone.toFixed(
      0
    )}, Price: ${resources.raw.stone.price.toFixed(2)}<br>`;

  processedInfo.innerHTML = `Processed:<br>
    Refined Coal: ${playerStats.processed.refinedCoal.toFixed(
      0
    )}, Price: ${resources.processed.refinedCoal.price.toFixed(2)}<br>
    Steel: ${playerStats.processed.steel.toFixed(
      0
    )}, Price: ${resources.processed.steel.price.toFixed(2)}<br>
    Stone Bricks: ${playerStats.processed.stoneBricks.toFixed(
      0
    )}, Price: ${resources.processed.stoneBricks.price.toFixed(2)}<br>`;
  ...
  inavrg.shops.stoneBricks =
    inavrg.shops.stoneBricks / incomeHistory.shops.stoneBricks.length;
...
productionTerminalButton.addEventListener("click", () => {
  ...
  FauAmp.addEventListener("input", () => {
    document.getElementById(
      "FouAmpLabel"
    )!.innerText = `Set Production Rate (%): ${FauAmp.value}%`;
  });
  MinAmp.addEventListener("input", () => {
    document.getElementById(
      "MinAmpLabel"
    )!.innerText = `Set Production Rate (%): ${MinAmp.value}%`;
  });
